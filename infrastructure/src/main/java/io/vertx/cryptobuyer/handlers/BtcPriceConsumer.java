...
import io.vertx.cryptobuyer.repository.models.PersistedOrder;
import io.vertx.cryptobuyer.repository.models.PersistedOrderKey;
import io.vertx.cryptobuyer.repository.models.PersistedOrderQuery;
...
public class BtcPriceConsumer extends Verticle {
  ...
  // this will transform order's into entries in the processing queue concurrently,
  // it scales quite well too. It keeps polling on order's until there's none with limit lower than current price
  ...
  private PgQueueProducer<PurchaseBtcCommand> queueProducer;
  ...
  private Uni<Void> pollUntilDone(BigDecimal currentBtcOPrice) {
    ...
    return pollOnce(currentBtcOPrice)
      .flatMap(orders -> {
          ...
          final var messages = orders.stream()
            ...
            .map(order -> new Message<>(
                ...
                null,
                0,
                new PurchaseBtcCommand(currentBtcOPrice, order)
              )
            )
            .toList();
  ...
  private static String pollingStatement() {
    return """
      update orders set state = 'PROCESSING' where id in (
